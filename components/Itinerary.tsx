...
import { MapPin, Clock, Trash2, Edit2, ChevronDown, ChevronUp, Calendar, CloudSun, Thermometer, Shirt, Check, X, TrainFront, RefreshCw, Share, Copy } from 'lucide-react';

export const Itinerary: React.FC<ItineraryProps> = ({ plans, onUpdatePlans, onReset, destination }) => {
  ...
  const handleExport = async () => {
    const lines = [`🌍 ${destination} 旅行计划`, ''];

    plans.forEach(day => {
      lines.push(`📅 ${day.date} (${day.dayOfWeek})`);
      if (day.weather) {
        lines.push(`   ⛅ ${day.weather.condition} | ${day.weather.temp} | 👕 ${day.weather.outfit}`);
      }
      lines.push('');
      ...
      day.items.forEach(item => {
        ...
        if (item.transport) lines.push(`      🚇 ${item.transport}`);
    ...
    lines.push('Generated by Trip Genius ✨');
    ...
    <div className="space-y-4 pb-20">
      <div className="flex justify-between items-center mb-2">
        <button
          ...
          className={`text-xs font-semibold flex items-center gap-1.5 px-3 py-1.5 rounded-full shadow-sm border transition-all active:scale-95 ${hasCopied
        ...
        <button
          onClick={onReset}
          className="text-xs font-medium text-slate-500 flex items-center gap-1 hover:text-red-500 transition-colors bg-white px-3 py-1.5 rounded-full shadow-sm border border-slate-100"
        >
          <RefreshCw size={12} />
          重新规划
        </button>
      ...
      {plans.map((day) => (
        <div key={day.id} className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden transition-all duration-300">
          <button
            ...
            className={`w-full flex items-center justify-between p-4 transition-colors ${expandedDay === day.id
                ? 'bg-blue-50/50'
                : 'bg-white hover:bg-slate-50'
              }`}
          >
            <div className="flex items-center gap-3">
              <div className={`rounded-lg p-2 shadow-sm transition-colors ${expandedDay === day.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'
                }`}>
                <Calendar size={20} />
              </div>
