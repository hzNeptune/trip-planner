...
import { CalendarDays, Sparkles, CheckSquare, Plane } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleResetPlanning = () => {
    // Immediate response without blocking confirm dialog
    setIsPlanning(true);
    // We don't clear plans here immediately, so user can "Cancel" and go back
  };
        ...
        <header className="bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b border-slate-100 px-6 py-4 transition-all">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium mt-0.5">Exploring {destination} 🌍</p>
            </div>
            
            {/* Clickable Avatar to Reset/Edit */}
            <button 
        ...
        <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white border-t border-slate-200 px-6 py-3 pb-6 z-20 flex justify-between items-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          <button
            ...
            className={`flex flex-col items-center gap-1 transition-colors ${
              activeTab === Tab.ITINERARY ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 transition-colors ${
              activeTab === Tab.CHECKLIST ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'
            }`}
          >
